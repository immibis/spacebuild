...
function TOOL:LeftClick( trace, worldweld )
	...
	if ( trace.Entity:IsValid() && trace.Entity:GetClass() == "gmod_emitter" && trace.Entity:GetTable().pl == ply ) then

		trace.Entity:GetTable():SetEffect( effect )
...
if (SERVER) then
	...
	function MakeEmitter( ply, Pos, key, delay, toggle, effect, starton, Vel, aVel, frozen, nocollide )
		...
		emitter:GetTable():SetEffect( effect )
		emitter:GetTable():SetPlayer( ply )
		
		emitter:GetTable():SetDelay( delay )
		emitter:GetTable():SetToggle( toggle )
		emitter:GetTable():SetOn( starton )
