...
function TOOL:RightClick( trace )
	...
	if ( trace.Entity:IsValid() && 
		 ...
		 trace.Entity:GetTable():GetPlayer() == ply ) then

		trace.Entity:GetTable():SetKey( key )
		trace.Entity:GetTable():SetLabel( description )
...
function TOOL:LeftClick( trace )
	...
	button:GetTable().nocollide = true
...
if (SERVER) then
	...
	function MakeButton( pl, Model, Ang, Pos, key, description, nocollide, Vel, aVel, frozen )
		...
		button:GetTable():SetPlayer( pl )
		button:GetTable():SetKey( key )
		button:GetTable():SetLabel( description )
