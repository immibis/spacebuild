...
function TOOL:LeftClick( trace )
	...
	then
		trace.Entity:GetTable():Setup( _damage )
		return true
...
if SERVER then
	...
	function MakeDynamite(pl, Pos, Ang, key, Damage, Vel, aVel, frozen )
		...
		dynamite:GetTable():Setup( Damage )
		dynamite:GetTable():SetPlayer( pl )
		...
		pl:GetTable().DynamiteKeys = pl:GetTable().DynamiteKeys or {}
		
		// We only want to add this once per player..
		if ( !pl:GetTable().DynamiteKeys[ key ] ) then
			pl:GetTable().DynamiteKeys[ key ] = numpad.OnDown( pl, key, "DynamiteBlow", key )
		end
