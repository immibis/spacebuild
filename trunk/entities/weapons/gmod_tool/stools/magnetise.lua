...
function TOOL:LeftClick( trace )
	...
	if (magnet && magnet:IsValid())	then
		...
		if (trace.Entity:GetTable().colour) then
			
			local col = trace.Entity:GetTable().colour		
			SetColour(self:GetOwner(), magnet, col.r, col.g, col.b, col.a, col.mode, col.fx)
			
		end
		
		if (trace.Entity:GetTable().material) then
		
			local mat = trace.Entity:GetTable().material.mat
