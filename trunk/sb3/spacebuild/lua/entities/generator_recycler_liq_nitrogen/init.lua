...
function ENT:Initialize()
	...
	if not (WireAddon == nil) then
		...
		self.Outputs = Wire_CreateOutputs(self.Entity, {"On", "Overdrive", "HotLiquidNitrogenUsage", "EnergyUsage", "LiquidNitrogenProduction" })
	else
...
function ENT:Proc_Water()
	...
	if not (WireAddon == nil) then Wire_TriggerOutput(self.Entity, "HotLiquidNitrogenUsage", math.Round(winc)) end
	if (energy >= einc and water >= winc and nitrogen >= winc) then
		...
		self:ConsumeResource("hot liquid nitrogen", winc)
		...
		if not (WireAddon == nil) then Wire_TriggerOutput(self.Entity, "LiquidNitrogenProduction", math.Round(winc)) end
	else
		self:TurnOff()
	end
end
