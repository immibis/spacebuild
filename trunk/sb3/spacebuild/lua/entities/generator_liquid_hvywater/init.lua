...
function ENT:Initialize()
	...
	if not (WireAddon == nil) then
		...
		self.Outputs = Wire_CreateOutputs(self.Entity, {"On", "Overdrive", "WaterUsage", "EnergyUsage", "HeavyWaterProduction" })
	else
...
function ENT:Proc_Water()
	...
	if not (WireAddon == nil) then Wire_TriggerOutput(self.Entity, "WaterUsage", math.Round(winc)) end
	if (energy >= einc and water >= winc) then
		...
		--if (wchance <= 1+(self.overdrive)) then 
			...
			if not (WireAddon == nil) then Wire_TriggerOutput(self.Entity, "HeavyWaterProduction", (math.ceil(HW_Increment *self:GetMultiplier()) * self.Multiplier) * (1 + (self.overdrive * 2))) end
		--end
	else
		self:TurnOff()
	end
