...
function ENT:OnEnvironment(ent)
	...
	//Msg("Distance : "..tostring(dist2).." - "..tostring(dist).. " - "..tostring(dist * dist).."\n");
	self.testsize = self.testsize or (self:GetSize() * self:GetSize())
	if dist < self.testsize then
			...
			elseif ent.environment:GetPriority() == self:GetPriority() then
				if ent.environment:GetSize() != 0 then
					if self:GetSize() <= ent.environment:GetSize() then
						...
						if dist < (pos - ent.environment:GetPos()):LengthSqr() then
							ent.environment = self
						end
					end
				else
