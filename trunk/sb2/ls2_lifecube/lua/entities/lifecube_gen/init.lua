...
function ENT:Initialize()
	...
	if not (WireAddon == nil) then
		...
		self.Inputs = Wire_CreateInputs(self.Entity, { "On", "Need Energy", "Need Air", "Need Coolant", "Need Heavy water", "Need water", "Need steam", "Need ZPE" })
		self.Outputs = Wire_CreateOutputs(self.Entity, { "On", "Energy Output", "Air Output", "Coolant Output", "Heavy water Output", "water Output", "steam Output", "ZPE Output" })
	end
...
function ENT:TurnOn()
	if (self.Active == 0) then
		self.Active = 1
		--self.Entity:EmitSound( "lifecube/poweron.wav" )
		if not (WireAddon == nil) then Wire_TriggerOutput(self.Entity, "On", 1) end
		self:SetOOO(1)
        self.Entity:SetColor( 255, 50, 0, 255 )
	end
end
...
function ENT:TurnOff()
	if (self.Active == 1) then
		self.Active = 0
		--self.Entity:StopSound( "lifecube/poweron.wav" )
		if not (WireAddon == nil) then 
			Wire_TriggerOutput(self.Entity, "On", 0)
			Wire_TriggerOutput(self.Entity, "Energy Output", 0)
            Wire_TriggerOutput(self.Entity, "Air Output", 0)
            Wire_TriggerOutput(self.Entity, "Coolant Output", 0)
            Wire_TriggerOutput(self.Entity, "Heavy water Output", 0)
            Wire_TriggerOutput(self.Entity, "water Output", 0)
            Wire_TriggerOutput(self.Entity, "steam Output", 0)
            Wire_TriggerOutput(self.Entity, "ZPE Output", 0)
		end
		self:SetOOO(0)
        self.Entity:SetColor( 10, 96, 255, 255 )
	end
...
function ENT:Repair()
	self.Entity:SetColor( 10, 96, 0, 255 )
...
function ENT:Extract_Energy()
	...
	if not (WireAddon == nil) then 
        Wire_TriggerOutput(self.Entity, "Energy Output", self.aEnergy)
        Wire_TriggerOutput(self.Entity, "Air Output", self.aAir)
        Wire_TriggerOutput(self.Entity, "Coolant Output", self.aCoolant)
        Wire_TriggerOutput(self.Entity, "Heavy water Output", self.aHeavy_water)
        Wire_TriggerOutput(self.Entity, "water Output", self.aWater )
        Wire_TriggerOutput(self.Entity, "steam Output", self.aSteam)
        Wire_TriggerOutput(self.Entity, "ZPE Output", self.aZPE)
    end
