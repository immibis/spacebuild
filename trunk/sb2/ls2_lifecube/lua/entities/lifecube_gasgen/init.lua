...
self.propane = propane_Increment
	...
	if not (WireAddon == nil) then
		...
		self.Inputs = Wire_CreateInputs(self.Entity, { "On", "Need methane", "Need nitrous", "Need nitrogen", "Need naturalgas", "Need propane" })
		self.Outputs = Wire_CreateOutputs(self.Entity, { "On", "methane Output", "nitrous Output", "nitrogen Output", "naturalgas Output", "propane Output"})
	end
	self.Entity:SetColor( 100, 200, 220, 255 )
end
...
function ENT:TurnOn()
	if (self.Active == 0) then
		...
		self:SetOOO(1)
        self.Entity:SetColor( 220, 200, 100, 255 )
	end
end
...
function ENT:TurnOff()
	if (self.Active == 1) then
		...
		if not (WireAddon == nil) then 
			...
			Wire_TriggerOutput(self.Entity, "methane Output", 0)
            Wire_TriggerOutput(self.Entity, "nitrous Output", 0)
            Wire_TriggerOutput(self.Entity, "nitrogen Output", 0)
            Wire_TriggerOutput(self.Entity, "naturalgas Output", 0)
            Wire_TriggerOutput(self.Entity, "propane Output", 0)
...
function ENT:Extract_Energy()
	...
	if not (WireAddon == nil) then 
        Wire_TriggerOutput(self.Entity, "methane Output", self.methane)
        Wire_TriggerOutput(self.Entity, "nitrous Output", self.nitrous)
        Wire_TriggerOutput(self.Entity, "nitrogen Output", self.nitrogen)
        Wire_TriggerOutput(self.Entity, "naturalgas Output", self.naturalgas)
        Wire_TriggerOutput(self.Entity, "propane Output", self.propane )
