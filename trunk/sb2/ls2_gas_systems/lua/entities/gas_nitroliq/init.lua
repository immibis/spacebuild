...
if not (WireAddon == nil) then
    ENT.WireDebugName = "Nitrogen Liquidizer"
end

function ENT:Initialize()
	self.Entity:SetModel("models/Gibs/airboat_broken_engine.mdl")
    self.BaseClass.Initialize(self)
    self.Entity:SetColor(0, 13, 110, 255)
    ...
    self.coolantprod = 200 -- Coolant production
	...
	if not (WireAddon == nil) then self.Outputs = Wire_CreateOutputs(self.Entity, { "On", "Overdrive", "Energy Consumption", "Nitrogen Consumption", "Coolant Production"}) end
...
function ENT:LiquidateNitro()
	if ( self.overdrive == 1 ) then
        ...
        self.nitro = math.ceil(self.nitrocon * self.overdrivefactor)
        self.coolant = math.ceil(self.coolantprod * self.overdrivefactor)
        ...
        self.nitro = self.nitrocon
        self.coolant = self.coolantprod
    end
    
    if not (WireAddon == nil) then
        Wire_TriggerOutput(self.Entity, "Energy Consumption", self.energy)
        Wire_TriggerOutput(self.Entity, "Nitrogen Consumption", self.nitro)
        Wire_TriggerOutput(self.Entity, "Coolant Production", self.coolant)
    end
	...
	if ( self:CanRun() ) then
        ...
        RD_SupplyResource(self.Entity, "coolant",self.coolant)
