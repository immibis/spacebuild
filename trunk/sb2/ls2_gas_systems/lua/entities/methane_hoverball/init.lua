...
if not (WireAddon == nil) then
    ENT.WireDebugName = "Methane Hoverball"
end
...
function ENT:Initialize()
	...
	self.Outputs = Wire_CreateOutputs(self.Entity, { "Zpos", "Xpos", "Ypos", "Speed", "Strength", "Resistance", "MethaneConsumption" })
end
...
function ENT:Think()
    ...
    self.methanecon = math.abs(math.floor( self.speed + self.strength + (self.ZVelocity/5) ))
    
    if (self:CanRun() && self:GetHoverMode() ) then
        RD_ConsumeResource(self.Entity, "methane", self.methanecon)
    else
        self:DisableHover()
	self.methanecon = 0
    end
    ...
    Wire_TriggerOutput(self.Entity, "MethaneConsumption", self.methanecon)
	return true
end

 function ENT:CanRun()
 
	local methane = RD_GetResourceAmount(self.Entity, "methane")	
	return (methane >= self.methanecon)
