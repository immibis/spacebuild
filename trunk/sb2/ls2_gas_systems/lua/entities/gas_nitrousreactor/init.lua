...
if not (WireAddon == nil) then
    ENT.WireDebugName = "Large Nitrous Oxide Reactor"
end
...
function ENT:Initialize()
    ...
    -- resource attributes
    self.energyprod = 2200 --Energy production
    self.nitrouscon = 95 -- nitrous consumption
	...
	if not (WireAddon == nil) then self.Outputs = Wire_CreateOutputs(self.Entity, { "On", "Overdrive", "Nitrous Oxide Consumption", "Energy Production"}) end
...
function ENT:GenerateEnergy()
	if ( self.overdrive == 1 ) then
        self.energy = math.ceil((self.energyprod + math.random(5,50)) * self.overdrivefactor)
	...
	if ( self:CanRun() ) then
        ...
        RD_SupplyResource(self.Entity, "energy",self.energy)
	...
	if not (WireAddon == nil) then
        ...
        Wire_TriggerOutput(self.Entity, "Nitrous Oxide Consumption", self.nitrous)
    end
