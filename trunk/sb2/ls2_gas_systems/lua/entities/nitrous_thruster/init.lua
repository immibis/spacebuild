...
if not (WireMod == nil) then
	ENT.WireDebugName = "Nitrous Thruster"
end
...
function ENT:Initialize()
	...
	self.Outputs = Wire_CreateOutputs(self.Entity, { "On", "Nitrous Consumption" })
end
...
function ENT:SetForce( force, mul )
	...
	local phys = self.Entity:GetPhysicsObject()
	if (!phys:IsValid()) then
		Msg("Warning: [nitrous_thruster] Physics object isn't valid!\n")
		return
	end
...
function ENT:CanRun()
	local nitrous = RD_GetResourceAmount(self.Entity, "nitrous")	
 ...
 function ENT:Think()
	...
	if not (WireAddon == nil) then
		Wire_TriggerOutput(self.Entity, "Nitrous Consumption", self.nitrouscon)
...
function ENT:UpdateTextOutput()
	...
	self.Entity:SetNetworkedInt( 8, nitrous)
end
