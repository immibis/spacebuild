...
function ENT:Scan()
	if ( RD_GetResourceAmount(self.Entity, "energy") >= math.floor(self.range/25) ) then
		RD_ConsumeResource(self.Entity, "energy", math.floor(self.range/25))
		local closeents = ents.FindInSphere(self.Entity:GetPos(), self.range)
		for _, check in pairs( closeents ) do
			if (check.IsAsteroid == 1) then
				local effectdata = EffectData()
					effectdata:SetOrigin( check:GetPos() )
					effectdata:SetStart( self.Entity:GetPos() + Vector(22,0,65) ) --move beam start to front of dish
