...
local Energy_Increment = 400
local Water_Increment = 100
local Steam_Increment = 300
...
function ENT:Proc_Water()
	...
	local winc = Steam_Increment + (self.overdrive*Steam_Increment)

	if (energy >= einc and steam >= winc) then
		if ( self.overdrive == 1 ) then
			DamageLS(self, math.random(2, 3))
		end
		RD_ConsumeResource(self, "energy", einc)
		RD_ConsumeResource(self, "steam", winc)
		RD_SupplyResource(self, "water", Water_Increment + (self.overdrive * Water_Increment ))
	else
		self:TurnOff()
	end
end
