...
function ENT:TransferResources( restype, resamount )
    ...
    elseif (restype == "redterracrystal") then --Red Terra Crystal
    	...
    	local trail = util.SpriteTrail(self.Entity, 0, Color(255,0,0), true, 80, 0, 2, 1/(60+0)*0.5,"trails/plasma.vmt")
    	self.damagemultiplier = 5
    elseif (restype == "greenterracrystal") then --Green Terra Crystal
		self.Entity:SetColor(0,255,0,255)
    	local trail = util.SpriteTrail(self.Entity, 0, Color(0,255,0), true, 80, 0, 2, 1/(60+0)*0.5,"trails/plasma.vmt")
    	self.damagemultiplier = 7
    elseif (restype == "terrajuice") then --Terrajuice
		self.Entity:SetColor(0,0,0,255)
    	local trail = util.SpriteTrail(self.Entity, 0, Color(0,0,0), true, 80, 0, 2, 1/(60+0)*0.5,"trails/plasma.vmt")
...
function ENT:PhysicsCollide(data, physobj )
	...
	if class and class == "rts_massdriver" then
		RD_SupplyResource(data.HitEntity, self.resourcetype, self.resourceamount)
		self.Entity:EmitSound( "AlyxEMP.Discharge", 100, 100)
		self.Entity:Remove()
	else
		-- EXPLODE! KABLOOIE!
		--rts_Explosion(damage, piercing, area, position, killcredit)
		rts_Explosion( self.resourceamount * self.damagemultiplier / 4,(5 + self.damagemultiplier/10) , self.resourceamount * self.damagemultiplier / 2,self.Entity:GetPos()+  self.Entity:GetUp() * 25,self.Activator)
