...
function ENT:Initialize()
	...
	self.Entity:SetNetworkedInt("Spread",self.Spread)
	
	-- Add wire support, of course
	if not (WireAddon == nil) then 
		self.Inputs = Wire_CreateInputs(self.Entity, { "On", "Multiplier","Spread" }) 
		self.Outputs = Wire_CreateOutputs(self.Entity, {"On", "Distance", "Attenuation" })
	end
...
function ENT:DoTrace()
	local Trace = util.QuickTrace( self.Entity:GetPos(), self.Entity:GetUp() * 30000, { self.Entity } )
    		...
    		for nr, prop in pairs( Entz ) do
				...
				if ((selectedprop:IsValid()) or (prop:IsPlayer()) or (prop:IsNPC())) then
					...
					if ((prop:IsPlayer()) or (prop:IsNPC())) then
						prop:TakeDamage((100-self.attenuation)/10*self.multiplier,self.Entity:GetOwner())
...
function ENT:Think()
	...
	if (self.on == 1) then 
		if (RD_GetResourceAmount(self.Entity, "energy") >= (100*self.multiplier)) then
			RD_ConsumeResource(self.Entity, "energy",100*self.multiplier)
			--self:DoTrace()
			...
			for _, Ent in pairs(tTargets) do
				--tAng = (Ent:GetPos() - self.Entity:GetPos()):Normalize():Angle()
				--tSum = math.abs(tAng.p) + math.abs(tAng.y) + math.abs(tAng.r)
				
				--We don't want to supply power to ourselves
				if not (self.Entity == Ent) then
					--Calculate the angle of deviation from the target axis
					--relative to the parent entity.
					tDist =  self.Entity:GetPos():Distance(Ent:GetPos())
					tDist2 =(self.Entity:GetPos()+self.Entity:GetUp()*tDist):Distance(Ent:GetPos())
	...
	self:SetOverlayText("Transmitter ("..self.state..")\nDist: "..math.Round(self.distance).."\n"..self.attenuation.."% Loss")
	if not (WireAddon == nil) then 
			Wire_TriggerOutput(self.Entity, "On", self.on)
			Wire_TriggerOutput(self.Entity, "Distance", self.distance)
			Wire_TriggerOutput(self.Entity, "Attenuation", self.attenuation)
	end	
