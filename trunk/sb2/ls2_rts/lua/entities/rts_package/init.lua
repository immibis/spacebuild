...
function ENT:SetWeight(restype,resamount)
	...
	RD_AddResource   (self.Entity,restype,resamount)
	RD_SupplyResource(self.Entity,restype,resamount)
...
function ENT:HealthCheck(ExplodeMod)
	if self.health <= CDSFudgeFix then
		...
		rts_Explosion( self.resourceamount * self.damagemultiplier / 20 * ExplodeMod,2 + self.damagemultiplier + (ExplodeMod * 10) , self.resourceamount * self.damagemultiplier / 10 * ExplodeMod,self.Entity:GetPos()+  self.Entity:GetUp() * 25,self.Activator)
		self.Entity:Remove()
	end
end
