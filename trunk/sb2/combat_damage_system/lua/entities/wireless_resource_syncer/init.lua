...
function ENT:Initialize()
	...
	for k, res in pairs(self.Resources) do
		RD_AddResource(self.Entity, res, 0)
	...
	if(WireAddon != nil) then
		...
		Wire_TriggerOutput(self.Entity, "Max Distance", self.MaxDistance)
		Wire_TriggerOutput(self.Entity, "Channel", self.channel)
	end
end
...
function ENT:TurnOn()
	...
	self.Active = 1	
	self.Entity:EmitSound(self.APCEngineStartSound)
...
function ENT:TurnOff()
	...
	self.Active = 0
	self.Entity:StopSound(self.APCEngineStartSound)
	self.Entity:EmitSound(self.APCEngineStopSound)
	self.BaseClass.UpdateActive(self)
	self:UpdateToolTip()
end
...
function ENT:UpdateToolTip()
	...
	self.Entity:SetNetworkedString("ToolTip1", self.PrintName..": "..Active)
	self.Entity:SetNetworkedString("ToolTip2", "\nSyncing: "..Syncing)
	self.Entity:SetNetworkedString("ToolTip3", "\nChannel: "..self.channel)
end
		...
		elseif(self.Pair and self.Pair.Active == 1) then
			...
			for k, res in pairs(self.Resources) do
				if(RD_GetNetworkCapacity(self.Pair, res) > 0) then
					...
					local ResourceAmount = RD_GetResourceAmount(self.Entity, res)
