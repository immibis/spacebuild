...
function ENT:Initialize()
	...
	if not (WireAddon == nil) then
		self.Inputs = Wire_CreateInputs(self.Entity, { "On", "ProtectVessel"})
		self.Outputs = Wire_CreateOutputs(self.Entity, { "On", "ProtectVessel" })
	end
end
...
function ENT:TriggerInput(iname, value)
	...
	elseif iname == "ProtectVessel" then
		...
		if value == 1 then
			self.UseRad = false
			Wire_TriggerOutput(self.Entity, "ProtectVessel", value);
...
function ENT:Sense()
	...
	if self.UseRad then
		self.BaseTable = ents.FindInSphere(self.Entity:GetPos(), 512)
	else
	...
	for _, ent in pairs(self.BaseTable) do
		if ent:IsValid() and ent.heat then
			local div = 1
			local dist = self.Entity:GetPos():Distance(ent:GetPos())
			if dist > 512  then
				div = 8
...
function ENT:GetShipParts()
	local entities = {}
	local consts = {}
	entities, consts = self:GetNextPart(self.Entity, entities, consts)
