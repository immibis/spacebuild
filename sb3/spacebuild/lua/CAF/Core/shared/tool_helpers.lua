...
function CAFToolSetup.MaxLimit()
	if TOOL.Limited == true and TOOL.Limit >= 0 then
		...
		elseif CLIENT and TOOL.DeviceNamePlural then language.Add( 'SBoxLimit_'..TOOL.LimitName, 'Maximum '..TOOL.DeviceNamePlural..' Reached' ) end
	end
end
				...
				if not devlist.hide then
					...
					for _, dev in pairs(devlist.devices) do
						if not dev.hide then
							...
							if cur_model		== dev.model
							and cur_type		== dev.type
							and cur_sub_type 	== dev.sub_type then
...
function CAFTool.RightClick( self, trace )
	...
	if self.GetExtraCCVars then
		...
		if trace.Entity.Setup then trace.Entity:Setup(Extra_Data) end
	end
