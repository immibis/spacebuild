...
function ENT:Think()										-- Entity Think Function ----------------------------------------------------------------
	...
	-- Wiremod Outputs
	if not (WireAddon == nil) then 
		Wire_TriggerOutput(self.Entity, "Packet Ready"		, _BoolToInt(self.WaitingForRelease)	)
		Wire_TriggerOutput(self.Entity, "Loading Packet"	, _BoolToInt(self.ChargingPacket)		)
