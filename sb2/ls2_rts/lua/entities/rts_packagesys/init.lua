...
function ENT:TriggerInput(iname, value)
	...
	if(iname == "Compress") then
		if((value == 1) and (self.reloadtime < CurTime())) then
			if (RD_GetResourceAmount(self.Entity, "energy") >  self.energyuse) then
				self.charging = 1
				self.counter = 0
...
function ENT:Think()
	...
	local tempvalue = RD_GetResourceAmount(self.Entity, tempvarname)
...
---------------------WIRE MOD OUTPUTS-------------------------------------------------
	if not (WireAddon == nil) then 
			Wire_TriggerOutput(self.Entity, "Ready", (self.reloadtime < CurTime()))
