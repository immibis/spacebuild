...
if not (WireMod == nil) then
	ENT.WireDebugName = "Ion Thruster"
end
...
function ENT:Initialize()
	...
	self.Outputs = Wire_CreateOutputs(self.Entity, { "On", "Energy Consumption", "Coolant Consumption" })
end
...
function ENT:SetForce( force, mul )
	...
	local phys = self.Entity:GetPhysicsObject()
	if (!phys:IsValid()) then
		Msg("Warning: [ion_thruster] Physics object isn't valid!\n")
		return
	end
...
function ENT:CanRun()
	...
	if (energy >= self.energycon and coolant >= self.coolantcon) then
		return true
	else
		return false
	end
 ...
 function ENT:Think()
	...
	self.coolantcon = math.ceil(self.force/self.coolantdiv)
	self.energycon = math.ceil(self.force/self.energydiv)
	...
	if not (WireAddon == nil) then
		...
		Wire_TriggerOutput(self.Entity, "Coolant Consumption", self.coolantcon)
...
function ENT:UpdateTextOutput()
	...
	self.Entity:SetNetworkedInt( 9, coolant)
end
