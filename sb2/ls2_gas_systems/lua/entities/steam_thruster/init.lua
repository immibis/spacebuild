...
if not (WireMod == nil) then
	ENT.WireDebugName = "Steam Thruster"
end
...
function ENT:Initialize()
	...
	self.Outputs = Wire_CreateOutputs(self.Entity, { "On", "Steam Consumption" })
end
...
function ENT:SetForce( force, mul )
	...
	local phys = self.Entity:GetPhysicsObject()
	if (!phys:IsValid()) then
		Msg("Warning: [steam_thruster] Physics object isn't valid!\n")
		return
	end
...
function ENT:CanRun()
	local steam = RD_GetResourceAmount(self.Entity, "steam")	
	
	if (steam >= self.steamcon) then
		return true
	else
		return false
	end
 ...
 function ENT:Think()
	...
	if not (WireAddon == nil) then
		Wire_TriggerOutput(self.Entity, "Steam Consumption", self.steamcon)
...
function ENT:UpdateTextOutput()
	...
	self.Entity:SetNetworkedInt( 8, steam)
end
