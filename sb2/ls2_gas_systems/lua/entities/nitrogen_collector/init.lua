...
if not (WireAddon == nil) then
    ENT.WireDebugName = "Nitrogen Collector"
end

local Ground = 1 + 0 + 2 + 8 + 32
local Pressure_Increment = 3
local Energy_Increment = 6

function ENT:Initialize()
	self.Entity:SetModel("models/props_c17/light_decklight01_off.mdl")
	...
	if not (WireAddon == nil) then
		...
		self.Inputs = Wire_CreateInputs(self.Entity, { "On", "Disable Use" })
...
function ENT:Pump_Nitrogen()
	self.energy = RD_GetResourceAmount(self, "energy")
	if (self.energy >= Energy_Increment) then
		local inc = 7 + math.random(1,15)
		RD_SupplyResource(self, "nitrogen", inc)
