...
local matRed	 = Material("cds/melt_effect")
...
function EFFECT:Init(data)
	...
	self.ParentEntity:SetMaterial(matRed)
...
function EFFECT:Render()
	...
	self.Entity:SetColor(255, 255, 255, Fraction*200)
	...
	cam.Start3D(Pos, EyeAngles())
		SetMaterialOverride(matRed)
			self.Entity:DrawModel()
		SetMaterialOverride(0)
		if (render.GetDXLevel() >= 80) then
			render.UpdateRefractTexture()
			matRefract:SetMaterialFloat("$refractamount", Fraction/32)
