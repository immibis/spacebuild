...
function ENT:Initialize()
	...
	if not (WireAddon == nil) then
		...
		self.Outputs = Wire_CreateOutputs(self.Entity, { "Temperature",  "On" })
	end
...
function ENT:Sense()
	...
	if tr.Entity and tr.Entity:IsValid() and tr.Entity.heat then
	...
	if not (WireAddon == nil) then
		Wire_TriggerOutput(self.Entity, "Temperature", self.entHeat)
	end
	RD_ConsumeResource(self, "energy", Energy_Increment)
end

function ENT:ShowOutput()
	self.Entity:SetNetworkedInt( 1, self.entHeat )
end
