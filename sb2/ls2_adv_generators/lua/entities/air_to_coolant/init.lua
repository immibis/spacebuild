...
local Pressure_Increment = 500
local Air_Increment = 200
local Energy_Increment = 250
...
function ENT:Pump_Coolant()
	...
	local einc = Energy_Increment + (self.overdrive*Energy_Increment*3)
        local winc = Air_Increment + (self.overdrive*Air_Increment*3)
	if (energy >= einc and air >= winc) then
		...
		RD_ConsumeResource(self, "air", winc)
