...
function ENT:Initialize()
	...
	self.Active = 0
	if not (WireAddon == nil) then self.Inputs = Wire_CreateInputs(self.Entity, { "Fire" }) end
			...
			local try = math.ceil(hitent.resource.yield/25)
				...
				//while (hitent.resource.yield > 0) do
					...
					if (raw_res.resource.rarity == 3) then
						raw_res:SetColor( 255, 0, 0, 255 )
					elseif (raw_res.resource.rarity == 2) then
						raw_res:SetColor( 0, 255, 0, 255 )
					elseif (raw_res.resource.rarity == 1) then
						raw_res:SetColor( 0, 0, 255, 255 )
					end
...
function ENT:Attack()
	if ( RD_GetResourceAmount(self.Entity, "energy") >= Energy_Increment ) then
...
function ENT:Think()
	...
	self.Entity:NextThink(CurTime() + Refire_Rate)
	return true
end
